...
int main(int argc, char **argv)
{
    ...
    cv::setIdentity(KF.processNoiseCov, cv::Scalar::all(.005));
...
//	Load the Cascades
	...
	while (ros::ok())
    {
        ...
        double dT = (ticks - preTick).toSec(); // Seconds
		...
		else if(found)
		{
			...
			p.x = static_cast<int>(kalmanCenter.x);
			p.y = static_cast<int>(kalmanCenter.y);
		}
		else
		{
